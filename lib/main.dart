...
class CryptoUtils {
  ...
  static String decryptRawPacket(Uint8List data) {
    ...
    final key = Key.fromBase16('36353636353635363636373536353537'); // "6566565666756557" as hex
    
    final encrypter = Encrypter(AES(key, mode: AESMode.ecb, padding: null));
    
    final List<double> results = [];
    
    // Process data in 16-byte chunks (AES block size)
    for (int c = 0; c < hidDataLen; c += 16) {
      final chunk = data.sublist(c, c + 16);
    ...
    return results.join(' '); // or format as needed
  }
}
